...
abstract class Logs {
    ...
    public static async loadLogs(count:number) { // loads the most recently created logs into Logs.logs[]
        const logRecords = (await await dbQuery.makeDBQuery(`SELECT log_id, camera_id, numberplate, entry_timestamp, exit_timestamp, vehicle_id, entry_image_base64, exit_image_base64, acknowleged, known_vehicle FROM "Log" ORDER BY log_id LIMIT $1;`, [count.toString()])).rows;
